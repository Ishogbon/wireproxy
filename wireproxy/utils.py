...
from wireproxy.thirdparty.mitmproxy.net.http import encoding as decoder

log = logging.getLogger(__name__)

ROOT_CERT = "ca.crt"
ROOT_KEY = "ca.key"
COMBINED_CERT = "wireproxy-ca.pem"
...
def get_upstream_proxy(options):
    ...
    Args:
        options: The wire proxy options.
...
def build_proxy_args(proxy_config: Dict[str, NamedTuple]) -> Dict[str, str]:
    ...
    Args:
        proxy_config: The proxy config parsed out of the Wire Proxy options.
...
def extract_cert(cert_name="ca.crt"):
    ...
    try:
        ...
        with open(Path(os.getcwd(), cert_name), "wb") as out:
            out.write(cert)
        log.info(
            "{} extracted. You can now import this into a browser.".format(cert_name)
        )
...
def extract_cert_and_key(dest_folder, cert_path=None, key_path=None, check_exists=True):
    ...
    single file called wireproxy-ca.pem in the specified destination
    folder.
    ...
    Args:
        ...
        cert_path: Optional path to the root certificate. When not supplied
            wire proxy's own root certificate will be used.
        key_path: Optional path to the private key. When not supplied
            wire proxy's own private key will be used. Note that the key
