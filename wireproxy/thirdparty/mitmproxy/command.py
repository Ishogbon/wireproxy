...
import wireproxy.thirdparty.mitmproxy.types
from wireproxy.thirdparty.mitmproxy import exceptions
from wireproxy.thirdparty.mitmproxy import command_lexer
from wireproxy.thirdparty.mitmproxy.command_lexer import unquote
...
def typename(t: type) -> str:
    ...
    to = wireproxy.thirdparty.mitmproxy.types.CommandTypes.get(t, None)
    if not to:
...
class Command:
    ...
    def __init__(
        ...
        for name, parameter in self.signature.parameters.items():
            ...
            if not wireproxy.thirdparty.mitmproxy.types.CommandTypes.get(
        ...
        if (
            self.return_type
            and not wireproxy.thirdparty.mitmproxy.types.CommandTypes.get(
    ...
    def call(self, args: typing.Sequence[str]) -> typing.Any:
        ...
        if ret is None and self.return_type is None:
            return
        typ = wireproxy.thirdparty.mitmproxy.types.CommandTypes.get(self.return_type)
        assert typ
...
class CommandManager:
    ...
    def parse_partial(
        ...
        next_params: typing.List[CommandParameter] = [
            CommandParameter("", wireproxy.thirdparty.mitmproxy.types.Cmd),
            CommandParameter("", wireproxy.thirdparty.mitmproxy.types.CmdArgs),
        ]
        ...
        for part in parts:
            if part.isspace():
                parsed.append(
                    ParseResult(
                        value=part,
                        type=wireproxy.thirdparty.mitmproxy.types.Space,
                        valid=True,
                    )
                )
                continue
            ...
            if expected and expected.kind is inspect.Parameter.VAR_POSITIONAL:
                ...
                expected = CommandParameter(
                    "", wireproxy.thirdparty.mitmproxy.types.Unknown
                )

            arg_is_known_command = (
                expected.type == wireproxy.thirdparty.mitmproxy.types.Cmd
                and part in self.commands
            )
            arg_is_unknown_command = (
                expected.type == wireproxy.thirdparty.mitmproxy.types.Cmd
                and part not in self.commands
            )
            command_args_following = (
                next_params
                and next_params[0].type == wireproxy.thirdparty.mitmproxy.types.CmdArgs
            )
            ...
            to = wireproxy.thirdparty.mitmproxy.types.CommandTypes.get(
                expected.type, None
            )
            valid = False
            if to:
    ...
    def execute(self, cmdstr: str) -> typing.Any:
        ...
        command_name, *args = [
            unquote(part.value)
            for part in parts
            if part.type != wireproxy.thirdparty.mitmproxy.types.Space
        ]
...
def parsearg(manager: CommandManager, spec: str, argtype: type) -> typing.Any:
    ...
    """
    t = wireproxy.thirdparty.mitmproxy.types.CommandTypes.get(argtype, None)
    if not t:
...
def argument(name, type):
    ...
    specific types such as wireproxy.thirdparty.mitmproxy.types.Choice, which we cannot annotate
