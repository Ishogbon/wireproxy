...
from wireproxy.thirdparty.mitmproxy.net import websockets, tcp
from wireproxy.thirdparty.mitmproxy.server.protocol import base
from wireproxy.thirdparty.mitmproxy.utils import strutils
from wireproxy.thirdparty.mitmproxy.websocket import WebSocketFlow, WebSocketMessage
...
class WebSocketLayer(base.Layer):
    ...
    def __init__(self, ctx, handshake_flow):
        ...
        if "Sec-WebSocket-Extensions" in handshake_flow.response.headers:
            if (
                ...
                in handshake_flow.response.headers["Sec-WebSocket-Extensions"]
            ):
        ...
        request = Request(
            ...
            target=handshake_flow.request.path,
        )
        ...
        data = self.connections[self.client_conn].send(
            AcceptConnection(extensions=server_extensions)
        )
        self.connections[self.server_conn].receive_data(data)
        assert isinstance(
            next(self.connections[self.server_conn].events()), events.AcceptConnection
        )
    ...
    def _handle_event(self, event, source_conn, other_conn, is_server):
        if isinstance(event, events.Message):
            ...
            return self._handle_close_connection(
    ...
    def _inject_messages(self, endpoint, message_queue):
        while True:
            try:
                ...
                data = self.connections[endpoint].send(
                    Message(data=payload, message_finished=True)
                )
                endpoint.send(data)
            except queue.Empty:
                break
    ...
    def __call__(self):
        self.flow = WebSocketFlow(
            self.client_conn, self.server_conn, self.handshake_flow
        )
        ...
        try:
            while not self.channel.should_exit.is_set():
                self._inject_messages(
                    self.client_conn, self.flow._inject_messages_client
                )
                self._inject_messages(
                    self.server_conn, self.flow._inject_messages_server
                )
            ...
            self.flow.error = flow.Error(
                "WebSocket connection closed unexpectedly by {}: {}".format(s, repr(e))
            )
