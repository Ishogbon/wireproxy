...
from wireproxy.thirdparty.mitmproxy import connections  # noqa
from wireproxy.thirdparty.mitmproxy import exceptions, flow, http
from wireproxy.thirdparty.mitmproxy.net import websockets
from wireproxy.thirdparty.mitmproxy.server.protocol import base
from wireproxy.thirdparty.mitmproxy.server.protocol.websocket import WebSocketLayer
from wireproxy.thirdparty.mitmproxy.utils import strutils
...
class UpstreamConnectLayer(base.Layer):
    ...
    def __init__(self, ctx, connect_request):
        ...
        self.server_conn = ConnectServerConnection(
            (connect_request.host, connect_request.port), self.ctx
        )
    ...
    def _send_connect_request(self):
        self.log(
            ...
            [
                ...
                "Connect to: {}:{}".format(
                    self.connect_request.host, self.connect_request.port
                ),
            ],
        )
        ...
        if resp.status_code != 200:
            raise exceptions.ProtocolException(
                "Reconnect: Upstream server refuses CONNECT request"
            )
    ...
    def change_upstream_proxy_server(self, address):
        self.log(
            "Changing upstream mitmproxy to {} (CONNECTed)".format(repr(address)),
            "debug",
        )
...
def validate_request_form(mode, request):
    ...
    if request.first_line_format not in allowed_request_forms:
        if (
            request.is_http2
            and mode is HTTPMode.transparent
            and request.first_line_format == "absolute"
        ):
...
class HttpLayer(base.Layer):
    ...
    def __call__(self):
        ...
        while True:
            flow = http.HTTPFlow(
                self.client_conn, self.server_conn, live=self, mode=self.mode.name
            )
    ...
    def handle_upstream_connect(self, f):
        ...
        if not f.response:
            self.establish_server_connection(
                f.request.host, f.request.port, f.request.scheme
            )
    ...
    def _process_flow(self, f):
        try:
            ...
            if (
                self.mode is HTTPMode.upstream or "socks" in self.config.options.mode
            ) and self.matches_no_proxy(f.request):
                self.set_server((f.request.host, f.request.port))
        ...
        if (
            self.config.options.mode.startswith("reverse:")
            and not self.config.options.keep_host_header
        ):
        ...
        try:
            ...
            if f.response.status_code == 101:
                ...
                is_websocket = websockets.check_handshake(
                    f.request.headers
                ) and websockets.check_handshake(f.response.headers)
    ...
    def send_error_response(self, code, message, headers=None) -> None:
        try:
            ...
            exceptions.NetlibException,
            h2.exceptions.H2Error,
            exceptions.Http2ProtocolException,
        ):
            self.log(
                "Failed to send error response to client: {}".format(message), "debug"
            )
    ...
    def change_upstream_proxy_server(self, address):
        ...
        if hasattr(self.ctx, "change_upstream_proxy_server"):
            ...
            self.log(
                "Changing upstream mitmproxy to {} (not CONNECTed)".format(
                    repr(address)
                ),
                "debug",
            )
        ...
        if self.mode is HTTPMode.regular or self.mode is HTTPMode.transparent:
            ...
            if tls:
                raise exceptions.HttpProtocolException(
                    "Cannot change scheme in upstream mitmproxy mode."
                )
    ...
    def apply_proxy_auth(self, f):
        ...
        if not auth:
            try:
                ...
                if auth:
                    auth = (
                        b"Basic" + b" " + base64.b64encode(strutils.always_bytes(auth))
                    )
            except AttributeError:
                pass
