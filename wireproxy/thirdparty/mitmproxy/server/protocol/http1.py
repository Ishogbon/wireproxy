...
from wireproxy.thirdparty.mitmproxy.server.protocol import http as httpbase
...
class Http1Layer(httpbase._HttpTransmissionLayer):
    ...
    def read_request_trailers(self, request):
        if "Trailer" in request.headers:
            ...
            self.log(
                "HTTP/1.1 request trailer headers are not implemented yet!", "warn"
            )
        return None
    ...
    def check_close_connection(self, flow):
        request_close = http1.connection_close(
            flow.request.http_version, flow.request.headers
        )
        response_close = http1.connection_close(
            flow.response.http_version, flow.response.headers
        )
        read_until_eof = (
            http1.expected_http_body_size(flow.request, flow.response) == -1
        )
        close_connection = request_close or response_close or read_until_eof
        if (
            flow.request.first_line_format == "authority"
