...
from wireproxy.thirdparty.mitmproxy import connections, exceptions
from wireproxy.thirdparty.mitmproxy.server import config  # noqa
...
class _LayerCodeCompletion:
    ...
    def __init__(self, **mixin_args):  # pragma: no cover
        ...
        self.ctx = None
        """@type: wireproxy.thirdparty.mitmproxy.server.protocol.Layer"""
...
class Layer(_LayerCodeCompletion):
    ...
    def __init__(self, ctx, **mixin_args):
        ...
        The root layer is passed a :py:class:`wireproxy.thirdparty.mitmproxy.server.RootContext` object,
        which provides access to
        :py:attr:`.client_conn <wireproxy.thirdparty.mitmproxy.server.RootContext.client_conn>`,
        :py:attr:`.next_layer <wireproxy.thirdparty.mitmproxy.server.RootContext.next_layer>`
    ...
    def __call__(self):
        ...
        Raises:
            ~wireproxy.thirdparty.mitmproxy.exceptions.ProtocolException: if an exception occurs.
...
class ServerConnectionMixin:
    ...
    def __check_self_connect(self):
        ...
        if address:
            ...
            self_connect = (
                address[1] == self.config.options.listen_port
                and address[0] in forbidden_hosts
            )
            if self_connect:
    ...
    def __make_server_conn(self, server_address):
        if (
            ...
            and self.config.options.upstream_bind_address == ""
        ):
    ...
    def set_server(self, address):
        ...
        if self.server_conn.connected():
            self.disconnect()
        self.log(
            "Set new server address: {}:{}".format(address[0], address[1]), "debug"
        )
    ...
    def connect(self):
        ...
        Raises:
            ~wireproxy.thirdparty.mitmproxy.exceptions.ProtocolException:
            if the connection could not be established.
        """
        if not self.server_conn.address:
            raise exceptions.ProtocolException(
                "Cannot connect to server, no server address given."
            )
        try:
