...
import wireproxy.thirdparty.mitmproxy.net.http
from wireproxy.thirdparty.mitmproxy import connections, exceptions, http  # noqa
from wireproxy.thirdparty.mitmproxy import flow
from wireproxy.thirdparty.mitmproxy.coretypes import basethread
from wireproxy.thirdparty.mitmproxy.net import tcp
from wireproxy.thirdparty.mitmproxy.net.http import headers, http2, url
from wireproxy.thirdparty.mitmproxy.server.protocol import http as httpbase, base
...
class SafeH2Connection(connection.H2Connection):
    ...
    def safe_send_headers(
        self, raise_zombie: Callable, stream_id: int, headers: headers.Headers, **kwargs
    ):
        with self.lock:
            raise_zombie()
    ...
    def safe_send_body(
        ...
        for chunk in chunks:
            ...
            while position < len(chunk):
                ...
                if self.local_flow_control_window(stream_id) < len(
                    frame_chunk
                ):  # pragma: no cover
                    self.lock.release()
                    time.sleep(0.1)
...
class Http2Layer(base.Layer):
    ...
    def _handle_request_received(self, eid, event):
        headers = wireproxy.thirdparty.mitmproxy.net.http.Headers(
    ...
    def _handle_trailers(self, eid, event, is_server, other_conn):
        trailers = wireproxy.thirdparty.mitmproxy.net.http.Headers(
            [[k, v] for k, v in event.headers]
        )
    ...
    def _handle_priority_updated(self, eid, event):
        if not self.config.options.http2_priority:
            self.log(
                "HTTP/2 PRIORITY frame suppressed. Use --http2-priority to enable forwarding.",
                "debug",
            )
            return True
        ...
        with self.connections[self.server_conn].lock:
            ...
            if (
                mapped_stream_id in self.streams
                and self.streams[mapped_stream_id].server_stream_id
            ):
    ...
    def _map_depends_on_stream_id(self, stream_id, depends_on):
        ...
        if (
            mapped_depends_on in self.streams
            and self.streams[mapped_depends_on].server_stream_id
        ):
    ...
    def _cleanup_streams(self):
        ...
        outdated_streams = [
            stream_id
            for stream_id, stream in zombie_streams
            if stream.zombie <= death_time
        ]
        ...
        try:
            while True:
                ...
                for conn in r:
                    source_conn = (
                        self.client_conn
                        if conn == self.client_conn.connection
                    ...
                    with self.connections[source_conn].lock:
                        ...
                        if (
                            self.connections[source_conn].state_machine.state
                        ...
                        for event in incoming_events:
                            if not self._handle_event(
                                event, source_conn, other_conn, is_server
                            ):
...
class Http2SingleStreamLayer(httpbase._HttpTransmissionLayer, basethread.BaseThread):
    ...
    class Message:
        def __init__(self, headers=None):
            self.headers: Optional[wireproxy.thirdparty.mitmproxy.net.http.Headers] = (
            ...
            self.queued_data_length = 0  # used to enforce wireproxy.thirdparty.mitmproxy's config.options.body_size_limit
            self.trailers: Optional[wireproxy.thirdparty.mitmproxy.net.http.Headers] = (
    ...
    def __init__(
        ...
        h2_connection,
        stream_id: int,
        request_headers: wireproxy.thirdparty.mitmproxy.net.http.Headers,
    ) -> None:
    ...
    def raise_zombie(self, pre_command=None):  # pragma: no cover
        connection_closed = (
            self.h2_connection.state_machine.state
            == h2.connection.ConnectionState.CLOSED
        )
        if self.zombie is not None or connection_closed:
            ...
            raise exceptions.Http2ZombieException(
                "Connection or stream already dead: {}, {}".format(
                    self.zombie, connection_closed
                )
            )
    ...
    def send_request_headers(self, request):
        ...
        while True:
            ...
            max_streams = self.connections[
                self.server_conn
            ].remote_settings.max_concurrent_streams
            if (
                self.connections[self.server_conn].open_outbound_streams + 1
                >= max_streams
            ):
        ...
        self.server_stream_id = self.connections[
            self.server_conn
        ...
        try:
            self.connections[self.server_conn].safe_send_headers(
                ...
                end_stream=(
                    False
                    if request.content or request.trailers or request.stream
                    else True
                ),
    ...
    def run(self):
        ...
        try:
            ...
            self.log(
                "Changing the Host server for HTTP/2 connections not allowed: {}".format(
                    e
                ),
                "info",
            )
