...
from wireproxy.thirdparty.mitmproxy.net import tls as net_tls
...
class TlsLayer(base.Layer):
    ...
    def __call__(self):
        ...
        if self._client_tls:
            ...
            try:
                self._client_hello = net_tls.ClientHello.from_file(
    ...
    def set_server_tls(
        self, server_tls: bool, sni: Union[str, None, bool] = None
    ) -> None:
        """
    ...
    def _establish_tls_with_server(self):
        ...
        try:
            ...
            if (
                self.client_conn.tls_established
                and self.client_conn.get_alpn_proto_negotiated()
            ):
            ...
            self.server_conn.establish_tls(
                sni=self.server_sni, alpn_protos=alpn, **args
            )
        ...
        proto = (
            self.alpn_for_client_connection.decode()
            if self.alpn_for_client_connection
            else "-"
        )
    ...
    def _find_cert(self):
        ...
        use_upstream_cert = (
            self.server_conn
            and self.server_conn.tls_established
            and self.config.options.upstream_cert
        )
