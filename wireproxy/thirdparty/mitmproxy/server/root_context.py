from wireproxy.thirdparty.mitmproxy import exceptions, log
from wireproxy.thirdparty.mitmproxy.net import tls
from wireproxy.thirdparty.mitmproxy.server import modes, protocol
from wireproxy.thirdparty.mitmproxy.server.protocol import http
...
class RootContext:
    ...
    Attributes:
        client_conn:
            The :py:class:`client connection <wireproxy.thirdparty.mitmproxy.connections.ClientConnection>`.
        channel:
            A :py:class:`~wireproxy.thirdparty.mitmproxy.controller.Channel` to communicate with the FlowMaster.
            Provides :py:meth:`.ask() <wireproxy.thirdparty.mitmproxy.controller.Channel.ask>` and
            :py:meth:`.tell() <wireproxy.thirdparty.mitmproxy.controller.Channel.tell>` methods.
        config:
            The :py:class:`mitmproxy server's configuration <wireproxy.thirdparty.mitmproxy.server.ProxyConfig>`
    """
    ...
    def log(self, msg, level, subs=()):
        ...
        full_msg = [
            "{}:{}: {}".format(
                self.client_conn.address[0], self.client_conn.address[1], msg
            )
        ]
        for i in subs:
            full_msg.append("  -> " + i)
