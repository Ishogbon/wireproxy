...
from wireproxy.thirdparty.mitmproxy.net.check import is_valid_host, is_valid_port
from wireproxy.thirdparty.mitmproxy.utils.strutils import always_str
...
def parse(url):
    ...
    port = (
        parsed.port
    )  # Returns None if port number invalid in Py3.5. Will throw ValueError in Py3.6
    if not port:
