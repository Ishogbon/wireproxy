...
import wireproxy.thirdparty.mitmproxy.net.http.url
from wireproxy.thirdparty.mitmproxy.coretypes import multidict
from wireproxy.thirdparty.mitmproxy.net.http import cookies, message, multipart
from wireproxy.thirdparty.mitmproxy.net.http.headers import Headers
from wireproxy.thirdparty.mitmproxy.utils.strutils import always_bytes, always_str
...
class Request(message.Message):
    ...
    def host(self, val: Union[str, bytes]) -> None:
        ...
        if self.data.authority:
            self.authority = wireproxy.thirdparty.mitmproxy.net.http.url.hostport(
    ...
    def url(self) -> str:
        ...
        return wireproxy.thirdparty.mitmproxy.net.http.url.unparse(
    ...
    def url(self, val: Union[str, bytes]) -> None:
        ...
        self.scheme, self.host, self.port, self.path = (
            wireproxy.thirdparty.mitmproxy.net.http.url.parse(val)
        )
    ...
    def pretty_host(self) -> str:
        ...
        if authority:
            return wireproxy.thirdparty.mitmproxy.net.http.url.parse_authority(
                authority, check=False
            )[0]
        else:
            return self.host
    ...
    def pretty_url(self) -> str:
        ...
        pretty_host, pretty_port = (
            wireproxy.thirdparty.mitmproxy.net.http.url.parse_authority(
                host_header, check=False
            )
        )
        pretty_port = (
            pretty_port
            or wireproxy.thirdparty.mitmproxy.net.http.url.default_port(self.scheme)
            or 443
        )
    ...
    def _get_query(self):
        ...
        return tuple(wireproxy.thirdparty.mitmproxy.net.http.url.decode(query))

    def _set_query(self, query_data):
        query = wireproxy.thirdparty.mitmproxy.net.http.url.encode(query_data)
    ...
    def query(self) -> multidict.MultiDictView:
        ...
        :py:class:`~wireproxy.thirdparty.mitmproxy.net.multidict.MultiDictView` object.
        """
    ...
    def cookies(self) -> multidict.MultiDictView:
        ...
        An empty :py:class:`~wireproxy.thirdparty.mitmproxy.net.multidict.MultiDictView` object
    ...
    def path_components(self):
        ...
        return tuple(
            wireproxy.thirdparty.mitmproxy.net.http.url.unquote(i)
            for i in path.split("/")
            if i
        )
    ...
    def path_components(self, components):
        components = map(
            lambda x: wireproxy.thirdparty.mitmproxy.net.http.url.quote(x, safe=""),
            components,
        )
    ...
    def _get_urlencoded_form(self):
        ...
        if is_valid_content_type:
            return tuple(
                wireproxy.thirdparty.mitmproxy.net.http.url.decode(
                    self.get_text(strict=False)
                )
            )
        return ()
    ...
    def _set_urlencoded_form(self, form_data):
        ...
        self.content = wireproxy.thirdparty.mitmproxy.net.http.url.encode(
    ...
    def urlencoded_form(self):
        """
        The URL-encoded form data as an :py:class:`~wireproxy.thirdparty.mitmproxy.net.multidict.MultiDictView`
    ...
    def _set_multipart_form(self, value):
        self.content = wireproxy.thirdparty.mitmproxy.net.http.multipart.encode(
            self.headers, value
        )
    ...
    def multipart_form(self):
        """
        The multipart form data as an :py:class:`~wireproxy.thirdparty.mitmproxy.net.multidict.MultiDictView`
        object. An empty multidict.MultiDictView if the content-type indicates non-form data or the content could
        not be parsed.
