...
import wireproxy.thirdparty.mitmproxy.options
from wireproxy.thirdparty.mitmproxy import certs, exceptions
from wireproxy.thirdparty.mitmproxy.contrib.kaitaistruct import tls_client_hello
...
def client_arguments_from_options(
    options: "wireproxy.thirdparty.mitmproxy.options.Options",
) -> dict:
...
class MasterSecretLogger:
    ...
    def __call__(self, connection, where, ret):
        ...
        done_previously_but_not_logged_yet = hasattr(
            connection, "_still_needs_masterkey"
        )
        if done_now or done_previously_but_not_logged_yet:
            with self.lock:
                ...
                try:
                    ...
                    AssertionError,
                    SSL.Error,
                ):  # careful: exception type changes between pyOpenSSL versions
...
class ClientHello:
    ...
    def from_file(cls, client_conn) -> "ClientHello":
        ...
        client_conn:
            The :py:class:`client connection <wireproxy.thirdparty.mitmproxy.connections.ClientConnection>`.
        Returns:
            :py:class:`client hello <wireproxy.thirdparty.mitmproxy.net.tls.ClientHello>`.
        """
        try:
